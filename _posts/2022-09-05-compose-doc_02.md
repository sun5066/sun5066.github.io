...
UI 로직은 UI 자체에서 제공하는 데이터에 작동하는 로직이다. 이는 UI 요소의 상태 또는 UI 데이터 소스(예: 권한
API나 [리소스](https://developer.android.com/reference/android/content/res/Resources?hl=ko))에 있을 수 있다. UI 로직을 활용하는 상태 홀더에는
일반적으로 다음 속성이 있다.

- UI 상태 생성 및 UI 요소 상태 관리
- `Activity` 재생성 시 유지되지 않음: UI 로직에서는 호스팅되는 상태 홀더는 종종 UI 자체의 데이터 소스에 종속되므로 구성 변경 시 이 정보를 유지하려고 하면 메모리 누수를 일으키는 경우가 많다. 상태 홀더가 구성 변경 시 데이터를 유지하려는 경우 `remembered` 함수로 만든 컴포저블 UI 요소 상태는 `Activity` 재생성 전반에 걸쳐 상태를 유지하기 위해 `rememberSaveable`에 위임